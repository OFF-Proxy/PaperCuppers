...
//ファイルパス
const PIECE_ON_SQUARE_IMG_PATH = [
    ["img/black_small.jpg", "img/black_middle.jpg", "img/black_large.jpg"], //player_id == 1
    ["img/white_small.jpg", "img/white_middle.jpg", "img/white_large.jpg"]  //player_id == 2
];

const PIECE_IN_HAND_IMG_PATH = [
    ["img/hand_black_small.png", "img/hand_black_middle.png", "img/hand_black_large.png"], //player_id == 1
    ["img/hand_white_small.png", "img/hand_white_middle.png", "img/hand_white_large.png"]  //player_id == 2
];
...
//サーバからの参加承認を待つ
function WaitParticipationApproval(){
    el_frame.innerHTML = '<div class="middle">マッチング要求中：サーバにマッチングを要求しています。</div>';
    setTimeout(ReOpen, 30000);
}
...
function StartGame(first_turn_player_id, other_player_name){
    ...
    //Begin: 外枠となる要素の生成
    el_frame.innerHTML = "";
    el_frame.insertAdjacentHTML("beforeend", '<div class="bg_pattern Diagonal"></div>');
    ...
    for (const value_row of el_squares) {
        for (const value_column of value_row) {
            value_column.innerHTML = "<img src='" + SQUARE_IMG_PATH + "' class='square_img'>";
        }
    }
    ...
    for(let i = 0; i < 3; i++){
        let img = '<img src="' + PIECE_IN_HAND_IMG_PATH[my_player_id - 1][i] + '" onclick="SelectPieceFromHand(' + (i + 1) + ')" class="hand_img">';
    ...
    for(let i = 0; i < 3; i++){
        let img = '<img src="' + PIECE_IN_HAND_IMG_PATH[other_player_id - 1][i] + '" class="hand_img">';
...
function PlacePiece(player, piece_size, position) {
    ...
    el_squares[position[0]][position[1]].innerHTML = '<img src="' + PIECE_ON_SQUARE_IMG_PATH[player - 1][piece_size - 1] + '" class="square_img">';
...
function EndGame(end_message){
    ...
    el_frame.insertAdjacentHTML("beforeend", '<div id="back_button_line"></div>');
    let el_back_button_line = document.getElementById("back_button_line");
    el_back_button_line.insertAdjacentHTML("beforeend", '<button type="button" onclick="location.href=\'game_client.php\'" class="back_button">マッチング画面に戻る</button>');
    el_back_button_line.insertAdjacentHTML("beforeend", '<button type="button" onclick="location.href=\'../../mode.html\'" class="back_button">ゲーム選択画面に戻る</button>');
}
